...
function App() {
  ...
  async function send() { if (!active || !text.trim()) return; const body = text.trim(); setText(''); localStorage.removeItem(`v2-draft-${active.id}`); try { const data = await api(`/api/v2/chats/${active.id}/messages`, { method: 'POST', body: JSON.stringify({ body, clientId: crypto.randomUUID() }) }); setMessages((current) => [...current, data.message]); await loadChats(); } catch (error) { setToast(error.message); setText(body); } }
