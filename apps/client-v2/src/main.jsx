...
function ForwardModal({ open, chats, activeId, onClose, onForward }) {
  const targets = chats.filter((chat) => Number(chat.id) !== Number(activeId) && chat.type !== 'rss');
  return <IonModal isOpen={open} onDidDismiss={onClose} breakpoints={[0, 0.75, 1]} initialBreakpoint={0.75}><IonHeader><IonToolbar><IonTitle>Forward to…</IonTitle><IonButtons slot="end"><IonButton onClick={onClose}><X size={18} /></IonButton></IonButtons></IonToolbar></IonHeader><IonContent><IonList lines="none">{targets.map((chat) => <IonItem button key={chat.id} onClick={() => onForward(chat)}><Avatar entity={chat} icon={chat.type === 'saved' ? '★' : chat.type === 'group' ? 'G' : undefined} /><IonLabel><h2>{chat.title}</h2><p>{chat.type}</p></IonLabel></IonItem>)}</IonList></IonContent></IonModal>;
}
...
function ChatInfoModal({ open, chat, info, user, onClose, onCreateInvite, onPreference, onBlockPeer }) {
  const peer = chat?.type === 'direct' ? (info?.members || []).find((member) => Number(member.id) !== Number(user?.id)) : null;
  return <IonModal isOpen={open} onDidDismiss={onClose} breakpoints={[0, 0.72, 1]} initialBreakpoint={0.72}><IonHeader><IonToolbar><IonTitle>Chat info</IonTitle><IonButtons slot="end"><IonButton onClick={onClose}><X size={18} /></IonButton></IonButtons></IonToolbar></IonHeader><IonContent className="ion-padding"><div className="info-hero"><Avatar entity={chat} icon={chat?.type === 'group' ? 'G' : chat?.type === 'saved' ? '★' : undefined} /><h2>{chat?.title}</h2><p>{chat?.type}</p></div><div className="info-actions"><IonButton fill="outline" onClick={() => onPreference('pinned')}>{chat?.pinned ? <PinOff size={16} /> : <Pin size={16} />}{chat?.pinned ? 'Unpin chat' : 'Pin chat'}</IonButton><IonButton fill="outline" onClick={() => onPreference('muted')}>{chat?.muted ? <Bell size={16} /> : <BellOff size={16} />}{chat?.muted ? 'Unmute' : 'Mute'}</IonButton><IonButton fill="outline" onClick={() => onPreference('archived')}><Archive size={16} />{chat?.archived ? 'Unarchive' : 'Archive'}</IonButton>{peer && <IonButton color="danger" fill="outline" onClick={() => onBlockPeer(peer.id)}><UserX size={16} />Block user</IonButton>}</div>{info?.pinnedMessage && <IonItem lines="none"><Pin size={16} /><IonLabel><h2>Pinned message</h2><p>{info.pinnedMessage.body || info.pinnedMessage.fileName || 'Attachment'}</p></IonLabel></IonItem>}<IonList>{(info?.members || []).map((member) => <IonItem key={member.id}><Avatar entity={{ displayName: member.display_name, username: member.username, avatarUrl: member.avatar_url }} /><IonLabel><h2>{member.display_name || member.username}</h2><p>{member.role}</p></IonLabel></IonItem>)}</IonList>{chat?.type === 'group' && <IonButton expand="block" onClick={onCreateInvite}>Create invite link</IonButton>}</IonContent></IonModal>;
}

function SettingsModal({ open, user, onClose, onLogout, onUserUpdate, themeMode, onThemeModeChange }) {
  ...
  const [toast, setToast] = useState('');
  useEffect(() => { if (!open) return; setProfile({ displayName: user?.displayName || '', username: user?.username || '', mobile: user?.mobile || '', hidePresence: Boolean(user?.hidePresence) }); api('/api/v2/privacy').then((data) => setPrivacy({ readReceipts: Boolean(data.privacy?.read_receipts), lastSeen: data.privacy?.last_seen || 'everyone', allowMessages: data.privacy?.allow_messages || 'everyone' })).catch(() => {}); api('/api/v2/sessions').then((data) => setSessions(data.sessions || [])).catch(() => {}); if (isAdmin(user)) { api('/api/admin/users').then((data) => setAdminUsers(data.users || [])).catch((error) => setToast(error.message)); api('/api/admin/chats').then((data) => setAdminChats(data.chats || [])).catch(() => {}); } }, [open, user?.id]);
  ...
  async function savePrivacy() { try { await api('/api/v2/privacy', { method: 'PATCH', body: JSON.stringify(privacy) }); setToast('Privacy saved'); } catch (error) { setToast(error.message); } }
  async function changePassword() { try { await api('/api/me/password', { method: 'PATCH', body: JSON.stringify(password) }); setPassword({ currentPassword: '', newPassword: '' }); setToast('Password changed'); } catch (error) { setToast(error.message); } }
  async function logoutSession(session) { try { await api(`/api/v2/sessions/${session.id}`, { method: 'DELETE' }); setSessions((list) => list.filter((item) => item.id !== session.id)); } catch (error) { setToast(error.message); } }
  ...
  return <IonModal isOpen={open} onDidDismiss={onClose}><IonHeader><IonToolbar><IonTitle>Settings</IonTitle><IonButtons slot="end"><IonButton onClick={onClose}><X size={18} /></IonButton></IonButtons></IonToolbar><IonToolbar><IonSegment value={tab} onIonChange={(e) => setTab(e.detail.value)}><IonSegmentButton value="profile">Profile</IonSegmentButton><IonSegmentButton value="appearance">Appearance</IonSegmentButton><IonSegmentButton value="privacy">Privacy</IonSegmentButton><IonSegmentButton value="security">Security</IonSegmentButton><IonSegmentButton value="sessions">Sessions</IonSegmentButton>{isAdmin(user) && <IonSegmentButton value="admin">Admin</IonSegmentButton>}</IonSegment></IonToolbar></IonHeader><IonContent className="ion-padding">{tab === 'profile' && <div className="settings-card form-card"><Avatar entity={user} /><IonInput label="Display name" labelPlacement="stacked" value={profile.displayName} onIonInput={(e) => setProfile({ ...profile, displayName: e.detail.value || '' })} /><IonInput label="Username" labelPlacement="stacked" value={profile.username} onIonInput={(e) => setProfile({ ...profile, username: e.detail.value || '' })} /><IonInput label="Mobile" labelPlacement="stacked" value={profile.mobile} onIonInput={(e) => setProfile({ ...profile, mobile: e.detail.value || '' })} /><IonItem lines="none"><IonToggle checked={profile.hidePresence} onIonChange={(e) => setProfile({ ...profile, hidePresence: e.detail.checked })}>Hide presence</IonToggle></IonItem><IonButton expand="block" onClick={saveProfile}>Save profile</IonButton><IonButton color="danger" expand="block" onClick={onLogout}><LogOut size={16} /> Sign out</IonButton></div>}{tab === 'appearance' && <div className="settings-card form-card"><h2>{themeMode === 'dark' ? <Moon size={18} /> : <Sun size={18} />} Appearance</h2><IonSelect label="Theme" labelPlacement="stacked" value={themeMode} onIonChange={(e) => onThemeModeChange(e.detail.value)}><IonSelectOption value="system">System</IonSelectOption><IonSelectOption value="light">Light</IonSelectOption><IonSelectOption value="dark">Dark</IonSelectOption></IonSelect><div className="theme-preview"><div><b>Chat preview</b><p>This neutral palette is easier to read than the old green theme.</p></div><span>Aa</span></div></div>}{tab === 'privacy' && <div className="settings-card form-card"><h2>Privacy</h2><IonItem lines="none"><IonToggle checked={privacy.readReceipts} onIonChange={(e) => setPrivacy({ ...privacy, readReceipts: e.detail.checked })}>Read receipts</IonToggle></IonItem><IonSelect label="Last seen" labelPlacement="stacked" value={privacy.lastSeen} onIonChange={(e) => setPrivacy({ ...privacy, lastSeen: e.detail.value })}><IonSelectOption value="everyone">Everyone</IonSelectOption><IonSelectOption value="nobody">Nobody</IonSelectOption></IonSelect><IonSelect label="Who can message me" labelPlacement="stacked" value={privacy.allowMessages} onIonChange={(e) => setPrivacy({ ...privacy, allowMessages: e.detail.value })}><IonSelectOption value="everyone">Everyone</IonSelectOption><IonSelectOption value="contacts">Contacts</IonSelectOption></IonSelect><IonButton expand="block" onClick={savePrivacy}>Save privacy</IonButton></div>}{tab === 'security' && <div className="settings-card form-card"><h2><Lock size={18} /> Security</h2><IonInput label="Current password" labelPlacement="stacked" type="password" value={password.currentPassword} onIonInput={(e) => setPassword({ ...password, currentPassword: e.detail.value || '' })} /><IonInput label="New password" labelPlacement="stacked" type="password" value={password.newPassword} onIonInput={(e) => setPassword({ ...password, newPassword: e.detail.value || '' })} /><IonButton expand="block" disabled={password.newPassword.length < 8} onClick={changePassword}>Change password</IonButton></div>}{tab === 'sessions' && <IonList>{sessions.map((session) => <IonItem key={session.id}><IonLabel><h2>{session.id.slice(0, 10)}</h2><p>Last used: {formatDate(session.last_used_at)}</p></IonLabel><IonButton color="danger" fill="outline" onClick={() => logoutSession(session)}>Logout</IonButton></IonItem>)}</IonList>}{tab === 'admin' && <div className="admin-panel"><IonItem lines="none"><Shield size={18} /><IonLabel><h2>Admin panel</h2><p>{adminUsers.length} users · {adminChats.length} chats</p></IonLabel></IonItem><h3>Users</h3><IonList>{adminUsers.map((item) => <IonItem key={item.id}><Avatar entity={item} /><IonLabel><h2>{item.displayName || item.username}</h2><p>@{item.username} · {item.role} · {item.isBanned ? 'banned' : 'active'}</p></IonLabel><IonButton color={item.isBanned ? 'success' : 'danger'} fill="outline" disabled={item.id === user.id} onClick={() => toggleBan(item)}>{item.isBanned ? 'Unban' : 'Ban'}</IonButton></IonItem>)}</IonList><h3>Chats</h3><IonList>{adminChats.slice(0, 40).map((chat) => <IonItem key={chat.id}><IonLabel><h2>{chat.title || `${chat.type} #${chat.id}`}</h2><p>{chat.type} · owner #{chat.ownerId}</p></IonLabel></IonItem>)}</IonList></div>}</IonContent><IonToast isOpen={Boolean(toast)} message={toast} duration={2300} onDidDismiss={() => setToast('')} /></IonModal>;
}
...
function App() {
  ...
  async function forwardToChat(targetChat) { const message = forwardMessage; setForwardOpen(false); setForwardMessage(null); if (!message || !targetChat) return; try { await api(`/api/v2/messages/${message.id}/forward`, { method: 'POST', body: JSON.stringify({ chatId: targetChat.id }) }); setToast(`Forwarded to ${targetChat.title}`); await loadChats(); } catch (error) { setToast(error.message); } }
