...
function applyOnlineIds(setChats, onlineUserIds = []) {
  const onlineSet = new Set(onlineUserIds.map(Number));
  setChats((current) => current.map((chat) => (
    chat.peer
      ? { ...chat, peer: { ...chat.peer, isOnline: onlineSet.has(Number(chat.peer.id)) } }
      : chat
  )));
}
...
function App() {
    ...
    async function heartbeat() {
      try {
        const [presenceData, userData] = await Promise.all([
          api('/api/v2/presence/heartbeat', { method: 'POST', body: JSON.stringify({}) }),
          api('/api/me'),
        ]);
        if (!stopped) {
          applyOnlineIds(setChats, presenceData.onlineUserIds);
          setUser(userData.user);
        }
      } catch {
        // Socket state and cached data remain available when heartbeat fails.
      }
    }
  ...
  async function openChat(chat) {
    ...
    try {
      if (chat.type === 'rss') {
        const feed = await loadRssFeed(chat.rssUrl);
  ...
  async function voice() {
    ...
    try {
      ...
      setFeedError(requestError.message || 'Microphone access was denied');
    }
  }
